...
> 실습일: 2026-04-05 | 클러스터: myeks (EKS v1.34, ap-northeast-2) | 노드: t3.medium × 2

```bash
$ kubectl get nodes -o wide
NAME                                              STATUS   VERSION               INTERNAL-IP    OS-IMAGE
ip-192-168-2-91.ap-northeast-2.compute.internal   Ready    v1.34.4-eks-f69f56f   192.168.2.91   Amazon Linux 2023.10.20260302
ip-192-168-3-56.ap-northeast-2.compute.internal   Ready    v1.34.4-eks-f69f56f   192.168.3.56   Amazon Linux 2023.10.20260302

$ kubectl top node
NAME                                              CPU(cores)   CPU%   MEMORY(bytes)   MEMORY%
ip-192-168-2-91.ap-northeast-2.compute.internal   25m          1%     432Mi           13%
ip-192-168-3-56.ap-northeast-2.compute.internal   29m          1%     495Mi           15%
```
...
php-apache 앱에 50개 병렬 루프로 부하를 가해 실제 스케일링 흐름을 확인했다.

**스케일 아웃 (부하 → replica 증가):**

```
=== 22:29:28 (부하 시작 직후) ===
php-apache   cpu: 196%/50%   REPLICAS: 1

=== 22:29:38 ===
php-apache   cpu: 245%/50%   REPLICAS: 4   ← 4개로 즉시 증가

=== 22:29:50 ===
php-apache   cpu: 144%/50%   REPLICAS: 5

=== 22:30:01 ===
php-apache   cpu: 234%/50%   REPLICAS: 6

=== 22:30:24 ===
php-apache   cpu: 198%/50%   REPLICAS: 10  ← maxReplicas 도달
```

**스케일 인 (부하 제거 → 5분 안정화 윈도우 대기):**

```
=== 22:34:07 (부하 제거 직후) ===
php-apache   cpu: 0%/50%   REPLICAS: 10   ← CPU 0%지만 아직 대기 중

=== 22:39:03 (약 5분 후) ===
php-apache   cpu: 0%/50%   REPLICAS: 1    ← 스케일 인 완료
```
...
|------|--------|
| 스케일 아웃 소요 | ~30초 (1→10) |
| 스케일 인 소요 | ~5분 (stabilization window) |
| 트리거 임계값 | CPU 50% |
...
CPU를 지속적으로 소비하는 hamster 앱을 배포하고 VPA가 requests를 어떻게 조정하는지 확인했다. 초기 requests는 `cpu: 100m, memory: 50Mi`로 의도적으로 낮게 설정했다.

**설치 확인:**
```bash
$ kubectl get deploy -n kube-system | grep vpa
vpa-admission-controller   1/1     Running   29s
vpa-recommender            1/1     Running   32s
vpa-updater                1/1     Running   33s
```
...
hamster-vpa   Recreate   587m   250Mi   True       6m42s
```

```yaml
# kubectl describe vpa hamster-vpa 결과 발췌
status:
  recommendation:
    containerRecommendations:
    - containerName: hamster
      lowerBound:  { cpu: 380m, memory: 250Mi }
      target:      { cpu: 587m, memory: 250Mi }   # ← VPA 추천값
      upperBound:  { cpu: "1",  memory: 500Mi }
```

**Eviction 이벤트 (VPA Updater가 파드를 재생성):**
```
Normal  EvictedByVPA  pod/hamster-xxx-lg24g  Pod was evicted by VPA Updater to apply resource recommendation.
Normal  EvictedByVPA  pod/hamster-xxx-jxgvx  Pod was evicted by VPA Updater to apply resource recommendation.
```

**변경 전후 비교:**

| 항목 | 변경 전 (초기값) | 변경 후 (VPA 적용) |
|------|------|------|
| CPU requests | 100m | **587m** (+5.87x) |
| Memory requests | 50Mi | **250Mi** (+5x) |
| 파드 재시작 | - | 2회 (순차 eviction) |
| 추천 반영 시간 | - | ~1분 |
...
### 실습 결과 - KEDA Cron 트리거 (Scale-to-Zero)

php-apache를 Cron ScaledObject로 연결해 매 정각/15/30/45분에 1개, 매 5/20/35/50분에 0개로 자동 조정되는 것을 확인했다.

**설치 확인:**
```bash
$ kubectl get pods -n keda
NAME                                               READY   STATUS
keda-admission-webhooks-646d4f888-wcrvl            1/1     Running
keda-operator-5c75746585-mktk8                     1/1     Running
keda-operator-metrics-apiserver-7ff55ddcd6-hkm6h   1/1     Running
```

**Scale-to-Zero → Scale Up 관찰:**
```
=== 22:56:05 (비활성 구간) ===
ScaledObject: Active=False
Replicas: 0                         ← Scale-to-Zero 상태

=== 23:00:14 (Cron :00 활성화) ===
ScaledObject: Active=True           ← 트리거 발동!
pod/php-apache-xxx  ContainerCreating

=== 23:00:46 ===
pod/php-apache-xxx  Running         ← 파드 1개 실행 중 (~32초 소요)
```

**Scale Down (Cron 비활성 + cooldownPeriod 300초):**
```
=== 23:05:15 (Cron :05 비활성 진입) ===
ScaledObject: Active=False (Cooling down)
Replicas: 1                         ← cooldownPeriod 대기 중

=== 23:10:14 (cooldown 300초 경과) ===
Replicas: 0                         ← Scale-to-Zero 완료
```

**KEDA가 내부에서 생성하는 HPA (External Metrics 방식):**
```yaml
metrics:
- type: External
  external:
...
| 항목 | 실측값 |
|------|--------|
| Scale-to-Zero | 지원 (minReplicaCount: 0) |
| 스케일 업 소요 | ~32초 (0→1) |
| 스케일 다운 소요 | cooldownPeriod 300초 후 |
| HPA 메트릭 타입 | External (Resource가 아님) |
...
| 기능 | HPA | VPA | KEDA |
|------|-----|-----|------|
| 스케일 방향 | 수평 (파드 수) | 수직 (리소스 크기) | 수평 (파드 수) |
| Scale-to-Zero | 불가 (min=1) | N/A | **가능** |
| 메트릭 소스 | CPU/Memory/Custom | CPU/Memory 실사용량 | 60+ 이벤트 소스 |
| 파드 재시작 | 없음 | **있음** (eviction) | 없음 |
| 반응 속도 (실측) | ~30초 (1→10) | ~1분 (추천 반영) | ~32초 (0→1) |
| 스케일 다운 | 5분 안정화 윈도우 | eviction 후 재생성 | cooldownPeriod 후 |
| 주요 용도 | 트래픽 기반 스케일링 | 리소스 최적화·우사이징 | 이벤트/스케줄 기반 |

> **이번 실습에서 스킵한 섹션**: 관리형 노드 그룹(Graviton/Spot), CPA, CAS, Karpenter, Fargate는 별도 클러스터 구성 또는 추가 비용이 필요해 스킵했다. 개념 정리는 위 각 섹션 참고.
